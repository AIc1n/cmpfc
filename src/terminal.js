...
class Terminal {
    ...
    commandListURL = "src/load-commands.js"
    mobileKeyboardURL = "src/keyboard.js"
    defaultFileystemURL = "src/defaultFilesystem.js"
    sidePanelURL = "src/html/side-panel.js"
    ...
    parse(prompt) {
        ...
        if (!this.commandExists(commandText)) { 
            return {argOptions: [], parsingError: {
                message: "команда не найдена", tokenIndex: 0, tokenSpan: 0
            }}
        }
    ...
    getCorrectnessText(prompt, inputElement) {
        ...
        if (!this.commandExists(commandText)) { 
            return underlinePart(prompt.indexOf(commandText), commandText.length, `команда не найдена`)
        }
    ...
    async loadCommand(name, {force=false}={}) {
        ...
        let commandWindow = await this._loadScript(`src/commands/${name}.js`)
